...
def gap_test(lambdaGap, n, ln, lmax, sdpbPrec, procsPerNode, dualityGapThreshold, workingDir, mac):
    ...
    if ln == 1:
        ...
        normalization = (["0"] * (lmax + 1) +
                        list(map(remove_exponent, [(F(n, 2 * n + l)(0)) for l in range(0, lmax + 1, 2)])) +
                        list(map(remove_exponent, [(F(n, 2 * n + l)(0)) for l in range(1, lmax + 1, 2)])))
        objective = ["0"] * (2 * lmax + 2)
        polynomials = ([[["0"] if i != j else ["-1"] for i in range(lmax + 1)] + ([["0"]] * (lmax + 1)) for j in list(range(lmax + 1))] +
                       [[["0"] for l in range(lmax + 1)] +
                        [list(map(remove_exponent, (F(n, 2 * n + l).shift(-lambdaGap).coefficients))) for l in range(0, lmax + 1, 2)] +
                        [list(map(remove_exponent, (F(n, 2 * n + l).shift(-lambdaGap).coefficients))) for l in range(1, lmax + 1, 2)]] +
                       [[list(map(remove_exponent, (F(n, 2 * n + l).shift(-lambdaGap)).coefficients)) for l in range(0, lmax + 1, 2)] +
                        [list(map(remove_exponent, ((-F(n, 2 * n + l)).shift(-lambdaGap)).coefficients)) for l in range(1, lmax + 1, 2)] +
                        [list(map(remove_exponent, (Polynomial([(-(Decimal(ln) + Decimal(1)) / Decimal(ln))]) * F(n, 2 * n + l).shift(-lambdaGap)).coefficients)) for l in range(0, lmax + 1, 2)] +
                        [list(map(remove_exponent, (Polynomial([((Decimal(ln) - Decimal(1)) / Decimal(ln))]) * F(n, 2 * n + l).shift(-lambdaGap)).coefficients)) for l in range(1, lmax + 1, 2)]])
...
def binarySearch(lambdaMin, lambdaMax, n, ln, gapPrec, lmax, pythonPrec, sdpbPrec, procsPerNode, dualityGapThreshold, workingDir, mac):
    if lambdaMax - lambdaMin < gapPrec:
        ...
        if ln == 1:
            ...
            nGammas = (lmax + 2) // 2
            nDeltas = (lmax + 1) // 2
            alphas = coefficients[0 : nAlphas]
            betas = coefficients[nAlphas : lmax + 1]
            gammas = coefficients[lmax + 1 : lmax + nGammas + 1]
            deltas = coefficients[lmax + nGammas + 1 :]
            assert len(alphas) == nAlphas == len(gammas) == nGammas
            assert len(betas) == nBetas == len(deltas) == nDeltas
            assert nAlphas + nBetas + nGammas + nDeltas == 2 * lmax + 2
            jsonResults = {"n" : n,
                           "ln" : ln,
                           "lmax" : lmax,
                           "gapPrec" : float(gapPrec),
